...
<body>
                        ...
                        <div class="panel-body">
                            <table class="table datatable_simple">
                                ...
                                <tbody>
                 				...
                 				<c:forEach items="${requestScope.map['prodata']}" var="m">
	                                    <tr>
	                                        <td>${m.id }</td>
	                                        <td>${m.projectName }</td>
	                                        <td>${m.projectDate }</td>
	                                        <td><img alt="" src="${pageContext.request.contextPath}/upload/${m.projectImg }" width="240" height="70"></td>
	                                        <th>
			                        ...
			                        <div class="pagination-container">
			                        	<ul class="pagination">
			                        		<li class="PagedList-skipToNext">
			                        			<a href="${pageContext.request.contextPath}/project/QuerProjectServlet?EX_search=${requestScope.search }&current=${(requestScope.current-1)>0?requestScope.current-1:1}" rel="up">&lt;&lt;</a>
			                        		</li>
			                        		<c:if test="${(requestScope.current-1)>0}">
			                        			 <li>
			                        		     <a href="${pageContext.request.contextPath}/project/QuerProjectServlet?EX_search=${requestScope.search }&current=${requestScope.current-1}">${requestScope.current-1}</a>
			                        		     </li>
			                        		</c:if>
			                        		
			                        		<c:set var="curr" value="${requestScope.current<=requestScope.map['zy']?requestScope.current:requestScope.map['zy']}"></c:set>
			                        		<li class="active"><a href="${pageContext.request.contextPath}/project/QuerProjectServlet?EX_search=${requestScope.search }&current=${curr}">${curr}</a></li>
			                        		...
			                        		<li class="PagedList-skipToNext">
			                        		<a href="${pageContext.request.contextPath}/project/QuerProjectServlet?EX_search=${requestScope.search }&current=${(requestScope.current+1)>requestScope.map['zy']?requestScope.map['zy']:(requestScope.current+1)}" rel="next">>></a>
			                        		</li>
			                        		</ul>
			                        </div>
                        </div>
    ...
    <script>
        function Create() {
            //设置页面地址  编辑功能地址带id参数
            window.location.href = "${pageContext.request.contextPath}/project/projectServlet?flag=sel";
            return false;
        }
        //删除商品
        function Delete(id) {
            ...
            $("#deleteComfirm").unbind("click").click(function () {               
                ...
                window.location.href = "${pageContext.request.contextPath}/project/DelProjectServlet?bid="+id;
            })      
        }
        //修改商品
        function update(id) {
            //设置页面地址  编辑功能地址带id参数
            window.location.href = "${pageContext.request.contextPath}/project/UpdateProServlet?flag=selByID&bid="+ id;
            return false;
        }
        //查询
        function search() {
