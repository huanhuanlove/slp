...
<body>
                        ...
                        <div class="panel-body">
                            <table class="table datatable_simple">
                                ...
                                <tbody>
                 				...
                 				<c:forEach items="${requestScope.linksList }" var="links">
	                                    <tr>
	                                        <td>${links.id }</td>
	                                        <td>${links.name }</td>
	                                        <td>${links.url }</td>
	                                        <td><img alt="" src="${pageContext.request.contextPath}/newsImgs/${links.linkImg }" width="240" height="70"></td>
	                                        <th>
                        	...
                        	<div style="margin-left: 50px">
			                      	  每页${links.id }条数据&nbsp;&nbsp;共${links.id }条数据
			                        <div class="pagination-container">
			                        	<ul class="pagination">
			                        		<li class="PagedList-skipToNext">
			                        			<a href="${pageContext.request.contextPath}/links/QueryLinksServlet?EX_search=${requestScope.search }&current=${(requestScope.current-1)>0?requestScope.current-1:1}" rel="up">&lt;&lt;</a>
			                        		</li>
			                        		<c:if test="${(requestScope.current-1)>0}">
			                        			 <li>
			                        		     <a href="${pageContext.request.contextPath}/links/QueryLinksServlet?EX_search=${requestScope.search }&current=${requestScope.current-1}">${requestScope.current-1}</a>
			                        		     </li>
			                        		</c:if>

			                        		<c:set var="curr" value="${requestScope.current<=requestScope.map['zy']?requestScope.current:requestScope.map['zy']}"></c:set>
			                        		<li class="active"><a href="${pageContext.request.contextPath}/links/QueryLinksServlet?EX_search=${requestScope.search }&current=${curr}">${curr}</a></li>
			                        		...
			                        		<li class="PagedList-skipToNext">
			                        		<a href="${pageContext.request.contextPath}/links/QueryLinksServlet?EX_search=${requestScope.search }&current=${(requestScope.current+1)>requestScope.map['zy']?requestScope.map['zy']:(requestScope.current+1)}" rel="next">>></a>
			                        		</li>
			                        		</ul>
			                        </div>
                        </div>
    ...
    <script>
        function Create() {
            //设置页面地址  编辑功能地址带id参数
            window.location.href = "${pageContext.request.contextPath}/Content/links/LinksForm.jsp";
            return false;
        }
        ...
        function Delete(id) {
            ...
            $("#deleteComfirm").unbind("click").click(function () {
                ...
                window.location.href = "${pageContext.request.contextPath}/links/DelLinksServlet?lid="+id;
            })
        ...
        function update(id) {
            //设置页面地址  编辑功能地址带id参数
            window.location.href = "${pageContext.request.contextPath}/links/UpdateLinksServlet?flag=selByID&bid="+ id;
            return false;
        }
        //查询
        function search() {
